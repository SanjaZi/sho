   ...
   <div   >
    <ul className="flex flex-col">
      {fullCartHome.map((card) => (
        <li key={card.id}>
        <div className=" flex flex-row justify-between items-center  w-full h-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mt-20">
        <img className="p-8 rounded-t-lg cardImage" src={card.img} alt="" />
           ...
           <button className="text-white h-12 bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 mr-3	" onClick={() => handleRemoveFromCart(card.id, card)}>
            Remove
          </button>
    </div>
